...
/* ********************************************************************************************* */
// Prototypes needed 
void SetupFormat(WAVEFORMATEX *wf, int bitRate);			// Used by InitializePlayback()					
int WaitOnHeader(WAVEHDR *wh, char cDit);	// Used by Playbuffer()

int	InitializePlayback(int bitRate)
{
	int		rc;
	// set up the format structure, needed for playback (and recording)
	SetupFormat(&WaveFormat, bitRate);
...
static void SetupFormat(WAVEFORMATEX *wf, int sRate)
{
	wf->wFormatTag = WAVE_FORMAT_PCM;
	wf->nChannels = 1;
	wf->nSamplesPerSec = sRate*1000;
...
int PlayBuffer(short *piBuf, long lSamples)
{
	...
	mmErr = waveOutWrite(HWaveOut, &WaveHeader, sizeof(WAVEHDR)); // somethig wrong here
	// wait for completion
...
/* RECORDING FUNCTIONS */
/* ********************************************************************************************* */
int InitializeRecording(int bitRate)
{
	MMRESULT rc;
...
void getMessages(char * message) {
	...
	long int * quoteIndex = (long int *)malloc(sizeof(long int));
	int * quoteLength = (int *)malloc(sizeof(int));
	...
	if ((err = fopen_s(&file, "FortuneCookies.txt", "r")) != 0)
		...
		// call the function that retrieves messages from the file
		success = GetMessageFromFile(file, message, 140, numQuotes, quoteIndex, quoteLength);

		// if the function returns anything but 0, it failed. Tell the user and exit the program.
		if (success != 0) {
			printf("Error sending message from file. Program will close."); // display error message
			exit(1); // exit program
		}
	}
}